...
假设接口的地址为`abc.com:8080`，那么我们可以在脚手架的 config 中配置一个 serve 的启动域名；只要是一二级域名和端口号相同
...
接着我们启动本地 serve 服务，就会以这个域名和端口号运行，我们需要修改本机 hosts 文件，将该域名指向`127.0.0.1`

```
127.0.0.1 local.abc.com
```

如果你系统有 vpn 之类的代理，需要关闭
...
也是异步加载不会阻塞页面渲染，但是当 script 加载就绪后，就会根据在文档中的顺序优先执行代码

## Set 无法对哪些值去重

> Set 成员是唯一且无序的，没有重复值。
> 向 Set 中加入值的时候，不会进行类型转换，类似于精确运算符`===`，主要的区别在于**`NaN等于自身`**，==而精确运算符`===`判断 NaN 不等于自身。==
...
forEach(callback,thisArg)：用于对集合成员执行 callback
